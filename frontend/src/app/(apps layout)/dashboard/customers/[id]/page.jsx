...
const EditCustomerPage = () => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            // Clean empty string fields but keep boolean values (even false)
            const cleanData = {};
            ...
            await customerService.updateCustomer(params.id, cleanData);
            router.push('/dashboard/customers');
        } catch (err) {
            console.error('Update error:', err);
            console.error('Response:', err.response?.data);
            setError(err.response?.data?.detail || "Müşteri güncellenirken bir hata oluştu.");
        } finally {
            setSaving(false);
        }
    };
    ...
    if (loading) {
        return (
            <Container fluid className="py-5 text-center">
                ...
                <p className="mt-3 text-muted">Müşteri bilgileri yükleniyor...</p>
            </Container>
        );
    }
    ...
    return (
        <Container fluid className="py-4">
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 className="mb-1 fw-bold" style={{ color: '#0e1327' }}>Müşteriyi Düzenle</h2>
